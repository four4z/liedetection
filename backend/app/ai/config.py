...
# Absolute path to the app/ai directory — used by openpose_utils to chdir
# (config.py lives inside app/ai/, so its dirname IS the OpenPose root)
OPENPOSE_DIR = os.path.dirname(os.path.abspath(__file__))

# Hardware
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Target Data
TARGET_VIDEO = r"app/ai/VideoRLTRetry\trial_lie_054.mp4"
...
TEMP_ROOT   = os.path.join(_this_dir, "temp_processing")
